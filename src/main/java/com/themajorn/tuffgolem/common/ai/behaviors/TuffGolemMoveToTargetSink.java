...
public class TuffGolemMoveToTargetSink extends Behavior<TuffGolemEntity> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel serverLevel, TuffGolemEntity tuffGolem) {
        if (this.remainingCooldown > 0 || tuffGolem.isPetrified()) {
            ...
            if (!flag && this.tryComputePath(tuffGolem, walktarget, serverLevel.getGameTime()) && !tuffGolem.isPetrified()) {
    ...
    protected boolean canStillUse(ServerLevel serverLevel, TuffGolemEntity tuffGolem, long l) {
        if (this.path != null && this.lastTargetPos != null) {
            Optional<WalkTarget> optional = tuffGolem.getBrain().getMemory(MemoryModuleType.WALK_TARGET);
            PathNavigation pathnavigation = tuffGolem.getNavigation();
            return !pathnavigation.isDone() && optional.isPresent() && !this.reachedTarget(tuffGolem, optional.get()) && !tuffGolem.isPetrified();
        } else {
            return false;
        }
    }

    protected void stop(ServerLevel p_23601_, TuffGolemEntity p_23602_, long p_23603_) {
    ...
    protected void start(ServerLevel p_23609_, TuffGolemEntity p_23610_, long p_23611_) {
    ...
    protected void tick(ServerLevel p_23617_, TuffGolemEntity p_23618_, long p_23619_) {
